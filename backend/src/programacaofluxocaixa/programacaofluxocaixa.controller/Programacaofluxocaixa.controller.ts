...
class ProgramacaoFluxocaixaController {
  ...
  async listarTodosPorUsuarioId(req: Request, res: Response) {
    try {
      ...
      const todos =
        await programacaofluxocaixaService.listarTodosPorUsuarioId(idUsuario);
  ...
  async pesquisarPorCriterio(req: Request, res: Response) {
    try {
      ...
      const todos =
        await programacaofluxocaixaService.pesquisarPorCriterio(criterios);
      res.status(200).json(todos);
    } catch (error) {
      res.status(400).json(error.message);
    }
  ...
  async criarDupla(req: Request, res: Response) {
    try {
      const programacaofluxocaixa =
        await programacaofluxocaixaService.criarDupla(req.body);
      res.status(200).json(programacaofluxocaixa);
    } catch (error) {
      res.status(400).json(error.message);
    }
